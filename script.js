...
(function () {
    ...
    const FRIDGE_ACTION_TIMEOUT_MS = 15000;
    ...
    function buildRecoverySnapshot() {
        ...
        return {
            ...
            autoLoop: {
                ...
                energyRecoveryInProgress: Boolean(autoLoop?.energyRecoveryInProgress),
                energyRecoveryRetryAt: Number(autoLoop?.energyRecoveryRetryAt ?? 0),
                energyRecoveryState: autoLoop?.energyRecoveryState
                    ? serializableCopy(autoLoop.energyRecoveryState)
                    : null
            },
            cleaner: {
    ...
    function collectCurrentRoomObjects(maxDepth = 7, maxObjects = 8000) {
        const location =
            w.penzville?.city?.Context?.currentLocation ??
            null;
        ...
        const found = [];
        const queue = [{ value: location, depth: 0 }];
        // `_h.Jql` is a confirmed furniture-model collection. Seed it when
        // available, but keep the generic traversal for client updates.
        const roomModel = safeReadProperty(location, "_h");
        const knownFurniture = safeReadProperty(roomModel, "Jql");
        if (roomModel) {
            queue.push({ value: roomModel, depth: 0 });
        }
        if (knownFurniture) {
            queue.push({ value: knownFurniture, depth: 0 });
        }
        const seen = new WeakSet();

        while (queue.length > 0 && found.length < maxObjects) {
            const { value, depth } = queue.shift();
            if (
                !value ||
                (typeof value !== "object" && typeof value !== "function") ||
            ...
            for (const name of names) {
                if (["parent", "stage", "graphics", "__parent"].includes(name)) {
                    continue;
                }
    ...
    function fridgeCooldownState(model, nowSeconds = Math.floor(Date.now() / 1000)) {
        const lastProductionTime =
            Number(safeReadProperty(model, "lastProductionTime") ?? 0);
        const elapsed =
            Math.max(0, nowSeconds - lastProductionTime);
    ...
    function findFridgesInCurrentRoom() {
        ...
        for (const object of collectCurrentRoomObjects()) {
            const typeId =
                String(safeReadProperty(safeReadProperty(object, "shopItem"), "typeId") ?? "");

            if (typeId !== "refrigerator3") {
                continue;
            }

            const objectId =
                safeReadProperty(object, "objectId");

            if (objectId == null || seenIds.has(String(objectId))) {
                continue;
            }

            seenIds.add(String(objectId));
            const cooldown =
                fridgeCooldownState(object);

            rows.push({
                objectId: String(objectId),
                typeId,
                model: object,
                lastProductionTime: cooldown.lastProductionTime,
                eaterId: safeReadProperty(object, "eaterId") ?? null,
    ...
    function findRoomActionManager() {
        ...
        for (const object of collectCurrentRoomObjects(6, 5000)) {
            if (
                typeof safeReadProperty(object, "startAction") !== "function" ||
                typeof safeReadProperty(object, "finishAction") !== "function"
            ) {
                continue;
            }
            ...
            const typeName =
                safeObjectType(object);
            if (/RoomActionNetManager/i.test(typeName)) {
                return object;
            }
        }
    ...
    function roomActionEventMatches(event, objectId) {
        const values = [
            event,
            safeReadProperty(event, "data"),
            safeReadProperty(event, "params"),
            safeReadProperty(event, "action")
        ].filter(Boolean);

        for (const value of values) {
            const eventObjectId =
                safeReadProperty(value, "objectId") ??
                safeReadProperty(value, "id");
            const action =
                safeReadProperty(value, "action") ??
                safeReadProperty(value, "actionId") ??
                safeReadProperty(value, "name");

            if (
                eventObjectId != null &&
                String(eventObjectId) === String(objectId) &&
                (action == null || String(action) === "use")
            ) {
                return true;
            }
        }
    ...
    function installRoomActionListeners(manager, objectId, state) {
        const listeners = [];
        const dispatchers = [
            manager,
            w.penzville?.city?.Context?.currentLocation
            ...
            typeof safeReadProperty(value, "addEventListener") === "function"
        );
        const add = (dispatcher, type, key) => {
            const listener = event => {
                if (roomActionEventMatches(event, objectId)) {
                    state[key] = event ?? true;
                }
            };

            try {
                dispatcher.addEventListener(type, listener);
                listeners.push({ dispatcher, type, listener });
            } catch {}
        };

        for (const dispatcher of dispatchers) {
            add(dispatcher, "roomActionStarted", "started");
            add(dispatcher, "roomActionFinished", "finished");
        }

        return () => {
            for (const { dispatcher, type, listener } of listeners) {
                try {
    ...
    async function walkNearRoomObject(objectId) {
        const avatar =
        ...
        const target = collectCurrentRoomObjects(6, 5000).find(object =>
            String(safeReadProperty(object, "objectId") ?? "") === String(objectId) &&
            typeof safeReadProperty(object, "getInteractPoint") === "function"
        );
        ...
        const point =
            getTargetInteractionPoint(target, avatar);
        const action =
            point ? createWalkActionToPoint(avatar, point) : null;
    ...
    async function eatFromFridge(objectId) {
        const row =
            findFridgesInCurrentRoom().find(item =>
                String(item.objectId) === String(objectId)
            );
        ...
        if (!row) {
            return {
                success: false,
                objectId: String(objectId),
                reason: "fridge not found"
            };
        }

        const availability =
            fridgeCooldownState(row.model);
        if (!availability.ready) {
            return {
                success: false,
                objectId: row.objectId,
                reason: "fridge cooldown active",
        ...
        if (!manager) {
            return {
                success: false,
                objectId: row.objectId,
                reason: "RoomActionNetManager unavailable"
            };
        }

        const previousTimestamp =
            availability.lastProductionTime;
        const eventState = {
            started: false,
            finished: false
        };
        const removeListeners =
            installRoomActionListeners(manager, row.objectId, eventState);

        console.log(`[AVA FRIDGE] Using ${row.objectId}`);
        await walkNearRoomObject(row.objectId);
        ...
        try {
            ...
            if (startResult === false) {
                return {
                    success: false,
                    objectId: row.objectId,
                    reason: "startAction rejected"
                };
            }

            const started = await waitForHouseState(() => {
                const currentTimestamp =
                    Number(safeReadProperty(row.model, "lastProductionTime") ?? 0);
                return eventState.started || currentTimestamp > previousTimestamp;
            }, 5000, 100);

            if (!started) {
                console.warn(
                    `[AVA FRIDGE] roomActionStarted not observable for ${row.objectId}; continuing through the normal finish call`
                );
            }
            ...
            if (
                ...
                await new Promise(resolve => setTimeout(resolve, 750));
                const startedEvent =
                    eventState.started && eventState.started !== true
                        ? eventState.started
                        : null;
                const finishParams =
                    safeReadProperty(startedEvent, "params") ??
                    safeReadProperty(safeReadProperty(startedEvent, "data"), "params") ??
                    null;
                ...
                if (finishResult === false) {
                    return {
                        success: false,
                        objectId: row.objectId,
                        reason: "finishAction rejected"
                    };
                }
            }

            const finished = await waitForHouseState(() => {
                const currentTimestamp =
                    Number(safeReadProperty(row.model, "lastProductionTime") ?? 0);
                return currentTimestamp > previousTimestamp || eventState.finished;
            }, FRIDGE_ACTION_TIMEOUT_MS, 100);

            if (!finished) {
                return {
                    success: false,
                    objectId: row.objectId,
                    reason: "roomActionFinished timeout"
                };
            }
            ...
            console.log(
                updatedTimestamp > previousTimestamp
                    ? "[AVA FRIDGE] Use succeeded; lastProductionTime updated"
                    : "[AVA FRIDGE] Use succeeded; roomActionFinished received"
            );
            ...
            return {
                ...
                objectId: row.objectId,
                previousTimestamp,
                lastProductionTime: updatedTimestamp,
                confirmedBy: updatedTimestamp > previousTimestamp
                    ? "lastProductionTime"
                    : "roomActionFinished"
            };
        } catch (error) {
    ...
    async function eatAvailableFridges(count = 1) {
        const requested =
        ...
        console.log(
            `[AVA FRIDGE] Found ${rows.length} refrigerators, ${ready.length} ready`
        );
        ...
        for (const row of ready) {
            ...
            const useResult =
                await eatFromFridge(row.objectId);

            if (useResult.success) {
                result.eaten++;
                result.successfulObjectIds.push(row.objectId);
            } else {
                result.failures.push(useResult);
            }
        }

        if (result.eaten < requested) {
            result.failures.push({
                reason: "insufficient ready refrigerators",
                missing: requested - result.eaten
            });
        }
    ...
    w.__AVA_LIST_FRIDGES__ = function () {
        ...
        const table = rows.map(row => ({
            ...
            lastUse: row.lastProductionTime
                ? new Date(row.lastProductionTime * 1000).toISOString()
                : null,
    ...
    w.__AVA_EAT_AVAILABLE__ = function (count = 1) {
        return eatAvailableFridges(count);
    };
    ...
    function initializeMapCleaner() {
        ...
        const cleaner = {
            ...
            resume() {
                ...
                if (
                    this.pauseReason === "insufficient-ready-fridges" &&
                    autoLoop?.enabled &&
                    typeof autoLoop.resumeEnergyRecovery === "function"
                ) {
                    return autoLoop.resumeEnergyRecovery(this, energy);
                }
            ...
            prepareForEnergyRecovery() {
                const saved = {
                    currentMap: this.currentMap,
                    mapIndex: this._mapIndex,
                    cycleMode: w.__AVA_AUTO_CLEAN_LOOP__?.cycleMode ?? null,
                    nextCycleMode: w.__AVA_AUTO_CLEAN_LOOP__?.nextCycleMode ?? null,
                    interruptedTargetId: this.currentTarget
                        ? stableObjectId(this.currentTarget)
                        : null
                };
                ...
                this._restoreWrappedTargets();
                this._rejectManualInteraction("energy recovery started");
                this.busy = false;
                this.currentTarget = null;
                this._activeAction = null;
                this.paused = true;
            ...
            restoreForEnergyRecovery(saved, maps = []) {
                const mapId = String(saved?.currentMap ?? "");
                const normalizedMaps = Array.isArray(maps)
                    ? maps.map(String)
                    : [];
                if (!mapId || !normalizedMaps.includes(mapId)) {
                    cleanerWarn("Energy recovery has no valid interrupted map");
                    return false;
                }

                this._clearTimers();
                this._restoreWrappedTargets();
                this.config.maps = normalizedMaps.slice();
                this._mapIndex = Number(saved.mapIndex ?? normalizedMaps.indexOf(mapId));
                if (!Number.isInteger(this._mapIndex) || normalizedMaps[this._mapIndex] !== mapId) {
                    this._mapIndex = normalizedMaps.indexOf(mapId);
                }
                this.visitedMaps = normalizedMaps.slice(0, this._mapIndex);
                this.running = true;
                this.paused = true;
                this.pauseReason = "energy-recovery";
                this.busy = false;
                ...
                this._timers = [];
                this._wrappedTargets = [];
                this._recoveryTargetId = saved.interruptedTargetId ?? null;
                return true;
            },

            resumeAfterEnergyRecovery(saved) {
                const mapId =
                    String(saved?.currentMap ?? "");

                if (!this.running || !mapId) {
                    return Promise.resolve(false);
                }

                this._mapIndex = Number(saved.mapIndex ?? this._mapIndex);
                this.currentMap = mapId;
                this._recoveryTargetId = saved.interruptedTargetId ?? null;
                this.busy = false;
                this.currentTarget = null;
                ...
                cleanerLog(`[AVA ENERGY] Returning to ${mapId}`);
                if (typeof w.__AVA_GO_WORK__ !== "function" || !w.__AVA_GO_WORK__(mapId)) {
                    cleanerWarn(`[AVA ENERGY] Work return teleport failed: ${mapId}`);
                    return Promise.resolve(false);
                }
                ...
                return new Promise(resolve => {
                    ...
                    const check = () => {
                        ...
                        const hasObjects = discoverCleanableObjects(this.config).length > 0;
                        const ready = Boolean(
                            mapMatches &&
                            avatar &&
                            (avatar !== previousAvatar || hasObjects)
                        );
                        ...
                        if (ready) {
                            ...
                            if (performance.now() - stableSince >= this.config.mapStableMs) {
                                this.paused = false;
                                this.pauseReason = null;
                                ...
                                if (isCurrentWorkFinished()) {
                                    ...
                                    resolve({ resumed: true, skipped: true });
                                    return;
                                }

                                this._scheduleScan(0);
                                cleanerLog("[AVA ENERGY] Cleaner resumed");
                                resolve({ resumed: true, skipped: false });
                                return;
                            }
                        } else {
                            stableSince = 0;
                        ...
                        if (performance.now() - startedAt >= this.config.mapLoadTimeout) {
                            cleanerWarn(`[AVA ENERGY] Work return load timeout: ${mapId}`);
                            resolve(false);
                            return;
                        }

                        this._setTimer(check, 500);
                    };
            ...
            _pauseForEnergy(energy = this.getEnergy()) {
                ...
                if (
                    autoLoop?.enabled &&
                    typeof autoLoop.startEnergyRecovery === "function" &&
                    autoLoop.startEnergyRecovery(this, energy)
                ) {
                    return false;
                }
    ...
    function initializeAutoCleanLoop() {
        ...
        const loop = {
            ...
            energyRecoveryState: null,
            energyRecoveryRetryAt: 0,
            _timers: [],
            ...
            status() {
                return {
                    ...
                    energyRecoveryRetryAt: this.energyRecoveryRetryAt,
                    energyRecoveryState: this.energyRecoveryState
                        ? { ...this.energyRecoveryState }
                        : null
                };
            },
            ...
            startEnergyRecovery(cleaner, energy, restoredState = null) {
                if (
                    !this.enabled ||
                    this.energyRecoveryInProgress ||
                    Date.now() < this.energyRecoveryRetryAt
                ) {
                    return false;
                }

                const savedWork = restoredState?.savedWork ??
                    cleaner.prepareForEnergyRecovery();

                if (!savedWork?.currentMap) {
                    cleanerWarn("[AVA ENERGY] Cannot save interrupted work");
                    return false;
                }

                this.energyRecoveryInProgress = true;
                this.phase = "energy-recovery";
                this.energyRecoveryState = {
                    step: restoredState?.step ?? "go-home",
                    savedWork,
                    requested: 2,
                    eaten: Number(restoredState?.eaten ?? 0),
                    remaining: Math.max(0, 2 - Number(restoredState?.eaten ?? 0))
                };
                this._clearTimers();

                console.warn(
                    `[AVA ENERGY] Energy below threshold: ${formatEnergy(energy)}`
                );
                console.log(
                    `[AVA ENERGY] Saving interrupted work: ${savedWork.currentMap}`
                );
                requestRecoveryCheckpointSave("energy-recovery", true);

                this._runEnergyRecovery(cleaner).catch(error => {
                    this.energyRecoveryInProgress = false;
                    this.phase = "energy-recovery-failed";
                    this.energyRecoveryState = {
                        ...this.energyRecoveryState,
                        step: "failed",
                        error: String(error)
                    ...
                    cleaner.paused = true;
                    cleaner.pauseReason = "energy-recovery-failed";
                    console.error("[AVA ENERGY] Recovery failed", error);
                    requestRecoveryCheckpointSave("energy-recovery-failed", true);
                });

                return true;
            },

            resumeEnergyRecovery(cleaner, energy = cleaner?.getEnergy?.()) {
                if (Date.now() < this.energyRecoveryRetryAt) {
                    const waitMinutes = Math.ceil(
                        (this.energyRecoveryRetryAt - Date.now()) / 60000
                    );
                    cleanerWarn(
                        `[AVA ENERGY] Refrigerators are still cooling down; retry in about ${waitMinutes} minute(s)`
                    );
                    return cleaner?.status?.() ?? false;
                }

                const restoredState = this.energyRecoveryState;
                this.energyRecoveryRetryAt = 0;
                if (!restoredState) {
                    cleanerWarn("[AVA ENERGY] No paused recovery state is available");
                    return false;
                }

                return this.startEnergyRecovery(cleaner, energy, restoredState);
            },

            async _runEnergyRecovery(cleaner) {
                const state =
                    this.energyRecoveryState;

                if (!state) {
                    throw new Error("missing energy recovery state");
                }
                ...
                if (!homeResult?.success) {
                    throw new Error("house teleport or room switch timeout");
                }

                state.step = "eat";
                requestRecoveryCheckpointSave("energy-recovery-eating", true);
                const eatResult =
                    await eatAvailableFridges(state.remaining);
                state.eaten += eatResult.eaten;
                state.remaining = Math.max(0, state.requested - state.eaten);
                requestRecoveryCheckpointSave("energy-recovery-ate", true);

                console.log(
                    `[AVA ENERGY] Ate successfully ${state.eaten}/${state.requested} times`
                );
                ...
                const energy =
                    cleaner.getEnergy();
                if (energy) {
                    console.log(`[AVA ENERGY] Current energy: ${formatEnergy(energy)}`);
                }

                if (
                    state.eaten < state.requested &&
                    (!energy || energy.current < MIN_ENERGY_TO_ACT)
                ) {
                    const retrySeconds =
                        Number(eatResult.nextReadyInSeconds ?? 0);
                    this.energyRecoveryRetryAt = Date.now() + Math.max(
                        30 * 60 * 1000,
                        retrySeconds > 0 ? retrySeconds * 1000 : 0
                    );
                    this.energyRecoveryInProgress = false;
                    this.phase = "energy-recovery-paused";
                    state.step = "waiting-for-fridges";
                    cleaner.paused = true;
                    cleaner.pauseReason = "insufficient-ready-fridges";
                    console.warn(
                        `[AVA ENERGY] Insufficient ready fridges; retry after ${new Date(this.energyRecoveryRetryAt).toISOString()}`
                    );
                    requestRecoveryCheckpointSave("insufficient-ready-fridges", true);
                    return false;
                }

                state.step = "return-to-work";
                requestRecoveryCheckpointSave("energy-recovery-returning", true);
                const resumed =
                    await cleaner.resumeAfterEnergyRecovery(state.savedWork);

                if (!resumed) {
                    throw new Error("work return teleport failed");
                }

                this.energyRecoveryInProgress = false;
                this.energyRecoveryState = null;
                this.energyRecoveryRetryAt = 0;
                this.phase = "cleaning";
            ...
            const check = () => {
                ...
                if (recoverySystemsReady()) {
                    ...
                    const energyRecoveryState =
                        checkpoint.autoLoop.energyRecoveryState;
                    if (energyRecoveryState?.savedWork?.currentMap) {
                        const cleaner = w.__AVA_MAP_CLEANER__;
                        const restored = cleaner.restoreForEnergyRecovery(
                            energyRecoveryState.savedWork,
                            autoLoop.maps
                        );
                        if (!restored) {
                            crashRecoveryState.lastRecoveryError =
                                "cleaner refused energy recovery checkpoint";
                            crashRecoveryState.recoveryInProgress = false;
                            autoLoop.recoveryInProgress = false;
                        ...
                        autoLoop.energyRecoveryRetryAt = 0;
                        crashRecoveryState.recoveryInProgress = false;
                        autoLoop.recoveryInProgress = false;
                        const started = autoLoop.startEnergyRecovery(
                            cleaner,
                            cleaner.getEnergy(),
                            energyRecoveryState
                        );
                        if (!started) {
                            crashRecoveryState.lastRecoveryError =
                                "auto loop refused energy recovery checkpoint";
                            autoLoop._startCycleWhenReady();
                            resolve(false);
                            return;
                        }
    ...
    w.__AVA_HELP__ = function () {
        const commands = [
            ...
            {
                section: "House refrigerators",
                commands: "__AVA_LIST_FRIDGES__(), __AVA_EAT_FROM_FRIDGE__(objectId), __AVA_EAT_AVAILABLE__(count)"
            },
            {
